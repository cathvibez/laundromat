...
interface ItemCard { id; owner; type; shade }   // no `damp`: it is derived, see below
...
items, fresh cards, machine contents (including socks a blanket has stranded), power, markers and decks-in-play
...
- **The socks/blanket transform is NOT inside the pure function**, and this matters more
  since v10, because it now decides whether an item LEAVES the machine at all. It mutates a
...
- a **5,000-machine seeded random sweep** with attached cards (Bleach, Coloring,
  ...
  and both the `bleachKillsDark` and `sanitizerOwnerOnly` readings — the latter is no
  longer reachable from play (v10 made Sanitizer always machine-wide) but is retained in
  `ReckoningOpts` precisely so these 733 fixtures keep comparing against the oracle;
...
- The complete v8 ruleset: four-tier ladder, all filters, all seven specials, all four
  ...
  the circuit break, socks stranded by a blanket, mandatory loading, fresh→ready promotion,
